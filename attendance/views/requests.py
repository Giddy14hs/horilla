        ...
        if form.is_valid():
            ...
            if attendance.employee_id.employee_work_info.reporting_manager_id:
                ...
                notify.send(
                    ...
                    verb=(
                        f"{employee.employee_first_name} {user_last_name}'s "
                        f"attendance update request for {attendance.attendance_date} is created"
                    ),

                    verb_ar=(
                        f"تم إنشاء طلب تحديث الحضور لـ {employee.employee_first_name} "
                        f"{user_last_name }في {attendance.attendance_date}"
                    ),

                    verb_de=(
                        "Die Anfrage zur Aktualisierung der Anwesenheit von "
                        f"{employee.employee_first_name} {user_last_name} "
                        f"für den {attendance.attendance_date} wurde erstellt"
                    ),

                    verb_es=(
                        "Se ha creado la solicitud de actualización de asistencia para "
                        f"{employee.employee_first_name} {user_last_name} el {attendance.attendance_date}"
                    ),

                    verb_fr=(
                        "La demande de mise à jour de présence de "
                        f"{employee.employee_first_name} {user_last_name} pour le {attendance.attendance_date} a été créée"
                    ),
...
def approve_validate_attendance_request(request, attendance_id):
    ...
    notify.send(
        ...
        verb=(
            f"Your attendance request for "
            f"{attendance.attendance_date} is validated"
        ),

        verb_ar=(
            f"تم التحقق من طلب حضورك في تاريخ {attendance.attendance_date}"
        ),
        verb_de=(
            f"Ihr Anwesenheitsantrag für das Datum {attendance.attendance_date} wurde bestätigt"
        ),
        verb_es=(
            f"Se ha validado su solicitud de asistencia para la fecha {attendance.attendance_date}"
        ),
        verb_fr=(
            f"Votre demande de présence pour la date {attendance.attendance_date} est validée"
        ),
    ...
    if attendance.employee_id.employee_work_info.reporting_manager_id:
        ...
        notify.send(
            ...
            verb=(
                f"{employee.employee_first_name} {user_last_name}'s "
                f"attendance request for {attendance.attendance_date} is validated"
            ),

            verb_ar=(
                f"تم التحقق من طلب الحضور لـ {employee.employee_first_name} {user_last_name} في {attendance.attendance_date}"
            ),
            verb_de=(
                f"Die Anwesenheitsanfrage von {employee.employee_first_name} {user_last_name} für den {attendance.attendance_date} wurde validiert"
            ),
            verb_es=(
                f"Se ha validado la solicitud de asistencia de {employee.employee_first_name} {user_last_name} para el {attendance.attendance_date}"
            ),
            verb_fr=(
                f"La demande de présence de {employee.employee_first_name} {user_last_name} pour le {attendance.attendance_date} a été validée"
            ),
            redirect=reverse("request-attendance-view") + f"?id={attendance.id}",
            icon="checkmark-circle-outline",
        )
